...
export default {
  ...
  data() {
    return {
      loading: false,
      controls: {
        shottitle: this.typedevice ? this.typedevice.shottitle : '',
        title: this.typedevice ? this.typedevice.title : '',
        description: this.typedevice ? this.typedevice.description : ''
      },
      editorOption: {
        modules: {
          toolbar: [
      ...
      this.$refs.form.validate(async (valid) => {
        if (valid) {
          if (!this.typedevice) {
            try {
              this.loading = true
              await this.$axios.$post('/api/typedevice', {
                shottitle: this.controls.shottitle,
                title: this.controls.title,
                description: this.controls.description
              })
              this.loading = false
              await this.$router.push('/admin/typedevice')
            } catch (err) {
              this.loading = false
              this.$message.error(
            ...
            try {
              this.loading = true
              await this.$axios.$put(`/api/typedevice/${this.typedevice.id}`, {
                shottitle: this.controls.shottitle,
                title: this.controls.title,
                description: this.controls.description
              })
