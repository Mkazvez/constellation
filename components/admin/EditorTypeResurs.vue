...
export default {
  ...
  data() {
    return {
      loading: false,
      controls: {
        shottitle: this.typeresurs ? this.typeresurs.shottitle : '',
        title: this.typeresurs ? this.typeresurs.title : '',
        description: this.typeresurs ? this.typeresurs.description : ''
      },
      editorOption: {
        modules: {
          toolbar: [
      ...
      this.$refs.form.validate(async (valid) => {
        if (valid) {
          if (!this.typeresurs) {
            try {
              this.loading = true
              await this.$axios.$post('/api/typeresurs', {
                shottitle: this.controls.shottitle,
                title: this.controls.title,
                description: this.controls.description
              })
              this.loading = false
              await this.$router.push('/admin/typeresurs')
            } catch (err) {
              this.loading = false
              this.$message.error(
            ...
            try {
              this.loading = true
              await this.$axios.$put(`/api/typeresurs/${this.typeresurs.id}`, {
                shottitle: this.controls.shottitle,
                title: this.controls.title,
                description: this.controls.description
              })
